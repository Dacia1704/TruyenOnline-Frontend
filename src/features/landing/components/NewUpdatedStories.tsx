...
export function NewUpdatedStories() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError(null);
      try {
        const result = await getStories({ isPublished: true, size: 8, sortType: "UPDATED" });
        setStories(result.data ?? []);
      } catch {
        setError("Không tải được truyện mới cập nhật.");
      } finally {
        setLoading(false);
      }
    };
    ...
    <section className="pb-4 pt-8 px-6 bg-background dark:bg-slate-900/50">
      <div className="mx-auto max-w-6xl">
        <div className="flex items-end justify-between mb-6">
          <div>
            <h2 className="text-2xl sm:text-3xl font-bold text-foreground dark:text-white">Mới cập nhật</h2>
            <p className="mt-1 text-sm text-muted-foreground dark:text-slate-400">
              Những truyện được cập nhật gần đây.
            </p>
          </div>
          <Link
            href="/stories?sortType=UPDATED"
