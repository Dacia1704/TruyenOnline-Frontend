  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (chapterType === "NOVEL") {
        ...
        await updateChapter(chapterId, { isPublished: chapter?.isPublished });
      } else {
      ...
      router.push(`/uploader/stories/${params?.slug}/chapters`);
    } catch {
          ...
          {chapterType === "NOVEL" && editor && (
            <div>
              ...
              <div className="mt-2 rounded-lg border border-border bg-background">
                ...
                <EditorContent
                  editor={editor}
                  className="min-h-[300px] px-3 py-3 text-sm [&_.ProseMirror]:outline-none"
                />
              </div>
            </div>
          )}
